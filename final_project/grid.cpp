...
void draw_grid(grid_t* grid)
{
    ...
    for (column_counter = grid->upper_left_y; column_counter <= (grid->upper_left_y + grid->height);
         column_counter++) {
    ...
    // Draw Bottom of grid
    for (row_counter = grid->upper_left_x; row_counter <= (grid->upper_left_x + grid->width);
         row_counter++) {
...
void clear_inside_grid(grid_t* grid)
{
    ...
    for (int i = grid->upper_left_x + 1; i < (grid->width + grid->upper_left_x); i++) {
        ...
        int row_counter, column_counter;
        for (column_counter = grid->upper_left_y + 1;
             column_counter <= (grid->upper_left_y + grid->height);
             column_counter++) {
